"""
    ...
    Design:
    Performing de-identification is a "chatty" process as consisting of joins over joins over joins ... depending on
    the operations.
    e.g: In order to determine a person's race two joins are required, one that determines the number of races and
    the other if the races should be generalized.
    This chatty process to be performed for every individual and on a every field subject to de-identification.

    As a result of the nature of the de-identificatioin operations (chatty) and the limitations imposed by bigquery,
    we have implemented a query builder that will generate and build an SQL query that will implement the
    ...
    e.g:
        ...
        2. The second subset of records contains the dates (specified by concepts) and will be shifted (row based
        operation)
...
# We expect the program to be run as follows : python deid.py --i_dataset <input_dataset> --table <table_name>
# --config path-of-config.json --log
#
SYS_ARGS = {}
if len(sys.argv) > 1:
...
class Policy:
    ...
    class TERMS:
        ...
        OBSERVATION_FILTERS = {"race": 'Race_WhatRace', "gender": 'Gender', "orientation": 'Orientation',
                               "employment": '_EmploymentStatus', "sex_at_birth": 'BiologicalSexAtBirth_SexAtBirth',
                               "language": 'Language_SpokenWrittenLanguage', "education": 'EducationLevel_HighestGrade'}
...
class Shift(Policy):
    """
        This class will generate the SQL queries that will perform date-shifting against either a meta-table,
        relational table or a hybrid table.
        ...
        if name not in self.cache:
            try:
                ...
                if q:
                    ...
                    # We are dealing with a meta table (observation), We need to shift the dates of the
                    # observation_source_value that are of type date
                    #
                    # I assume the only meta-table is the observation table in the case of another one more
    ...
    def __get_shifted_fields(self, fields, dataset, table):
        ...
        for field in fields:
            ...
            # shifted_field = """
            #     DATE_SUB( DATE_SUB(DATE_SUB( CAST(:name AS DATE),INTERVAL :year YEAR),INTERVAL :month MONTH),
            #     INTERVAL :day DAY) as :name
            # """.replace(':name',field.name).replace(":year",str(year)).replace(":month",str(month)).replace(":day",
            # str(day))
            if field.field_type == 'DATE':
                shifted_field = """DATE_SUB(CAST(:name AS DATE), INTERVAL  (select date_shift from
                 :i_dataset.people_seed xii where xii.person_id = :table.person_id) DAY) as :name"""
            else:
                shifted_field = """TIMESTAMP_SUB(CAST(:name AS TIMESTAMP), INTERVAL  (select date_shift from
                 :i_dataset.people_seed xii where xii.person_id = :table.person_id) DAY) as :name"""
            shifted_field = shifted_field.replace(":name", field.name).replace(":table", table).replace(":i_dataset",
                                                                                                        dataset)
            # shifted_field = """
            #    DATE_SUB( CAST(:name AS DATE), INTERVAL (SELECT seed from :i_dataset.people_seed xii WHERE
            #    xii.person_id = :table.person_id) DAY) as :name
        ...
        Logging.log(subject=self.name(), action='shifting.dates', object=[field.name for field in fields],
                    value=[field.name for field in fields])
        return r  # ",".join(r)
    ...
    def can_do(self, dataset, table):
        ...
        if name not in self.cache:
            try:
                ...
                # The remove object has {columns:[],rows:[]} both of which should hold criteria for removal if true (
                # simple logic)
                #
                ...
                if p:
                    # _fields = [field.name for field in schema if field.name not in self.fields] #--fields that will
                    # be part of the projection
                ...
                Logging.log(subject=self.name(), object=name, action='can_do',
                            value={"drop.cols": remove_cols, "shift.cols": date_cols})
                if q:
                    #
                    # We are dealing with observation / meta table. Certain rows have to be removed due to the fact
                    # that it's a meta-table
                    #   - Date  because they will be shifted
                    #   - {Race,Gender,Ethnicity, Education, Employment, Language, Sexual Orientation} because they
                    #   will be generalized
                    ...
                    sql = sql + """
                        ...
                        WHERE observation_source_concept_id in (
                            ...
                            AND REGEXP_CONTAINS(TRIM(concept_code),'(:filter)') IS FALSE
                        )

                       
                    """.replace(":code", codes).replace(":vocabulary_id", self.vocabulary_id).replace(":filter",
                                                                                                      sql_filter)
                    #
                    ...
                    #
                    xsql = [sql]
                    args = {"client": self.client, "dataset": dataset, "table": table, "fields": _fields, "sql": "",
                            "concept_source_id": [], "vocabulary_id": "", "concept_class_id": []}
                    handler = Group(**args)
                    for key in Policy.TERMS.OBSERVATION_FILTERS:
                        ...
                        if len(r.keys()) > 0:
                            ...
                            _sql_ = "SELECT :fields  :shifted_date_columns FROM :i_dataset.:table WHERE " \
                                    "observation_source_concept_id in (SELECT concept_id FROM :i_dataset.concept " \
                                    "WHERE REGEXP_CONTAINS(concept_code,'(?i):key')) "
                            _sql_ = _sql_.replace(":fields", ",".join(ofields)).replace(":table", table).replace(":key",
                                                                                                                 key).replace(
                                ":i_dataset", dataset)
...
class Group(Policy):
    """
        ...
        The operations will apply on :
            ...
            - employment
            - language
        This is an inherently inefficient operation as a result of the design of the database that unfortunately has
        redudancies and semantic ambiguity (alas)
        As such this code will proceed case by case

        @TODO: Talk with the database designers to improve the design otherwise this code may NOT scale nor be
        extensible
    """
    ...
    def race(self):
        ...
        # sql = "SELECT person_id,count(*) FROM raw.observation WHERE observation_source_concept_id in (SELECT
        # concept_id FROM raw.concept WHERE REGEXP_CONTAINS(concept_code,'^Race_')) GROUP BY person_id HAVING count(
        # *) > 1"
        # r = self.client.query(sql).to_dataframe()
        # m_ids = ",".join([str(value) for value in r['person_id'].tolist()])
        #
        # We retrieve the identifiers of all the known races {black,white,asian,other} and anything that doesn't
        # belong will be other
        # @NOTE:
        #   For some unknown reason (poor design) it would appear that on tables like person concept_name holds the
        #   value of the race whereas in observation table concept_code holds the value of the race
        # This is an unacceptable inconcsistency that make make data broadly available with different representations
        # thus increasing the risk of re-identification.
        #

        field_name = "concept_name" if self.table == 'person' else 'concept_code'
        fields = self.fields
        sql = "SELECT concept_id,concept_code,concept_name from :dataset.concept WHERE REGEXP_CONTAINS(vocabulary_id," \
              "'(PPI|Race)') AND REGEXP_CONTAINS(concept_name,'(White|Black|Asian|Other Race)') is TRUE AND " \
              "REGEXP_CONTAINS(concept_name,'(Native|Pacific)') is FALSE"
        ...
        if self.table == 'person':
            ...
            p[
                "race_concept_id"] = "IF(race_concept_id not in (:_ids),:other_id,race_concept_id) as " \
                                     "race_concept_id".replace(
                ":_ids", _ids).replace(":other_id", other_id)
            p[
                "race_source_value"] = "IF(race_concept_id not in (:_ids),':other_name',race_source_value) as " \
                                       "race_source_value".replace(
                ":_ids", _ids).replace(":other_name", other_name)
            ...
            mr_sql = "SELECT person_id from (SELECT COUNT(*), person_id FROM :dataset.:table WHERE " \
                     "observation_source_value like 'Race_%' GROUP BY person_id HAVING COUNT(*) > 1)".replace(
                ":dataset", self.dataset).replace(":table", self.table)
            p[
                'value_as_string'] = "IF( person_id in (:mr_sql),'Multi-Racial',IF(value_source_concept_id not in (" \
                                     ":_ids),':other_name',value_as_string)) as value_as_string".replace(
                ":_ids", _ids).replace(":other_name", other_name).replace(":mr_sql", mr_sql)
            p[
                'observation_source_concept_id'] = "IF(person_id in (:mr_sql),2000000,IF(value_source_concept_id not " \
                                                   "in (:_ids),:other_id,observation_source_concept_id)) as " \
                                                   "observation_source_concept_id".replace(
                ":_ids", _ids).replace(":other_id", other_id).replace(":mr_sql", mr_sql)
            # p['observation_source_value'] = "IF((SELECT COUNT(*) FROM :dataset.observation z WHERE
            # z.observation_source_value like 'Race_%' AND z.person_id = person_id) > 1,:other_name,
            # IF(value_source_concept_id not in (:_ids), ':other_name',observation_source_value)) as
            # observation_source_value".replace(":_ids",_ids).replace(":other_name",other_name).replace(":dataset",
            # self.dataset)
            # p['value_source_concept_id'] = "IF(value_source_concept_id not in (:_ids), ':other_id',
            # value_source_concept_id) as value_source_concept_id".replace(":_ids",_ids).replace(":other_name",
            # other_name).replace(":dataset",self.dataset)
            p[
                'value_source_concept_id'] = "IF(person_id in (:mr_sql),2000000,IF(value_source_concept_id not in (" \
                                             ":_ids),:other_id,value_source_concept_id)) as " \
                                             "value_source_concept_id".replace(
                ":_ids", _ids).replace(":other_id", other_id).replace(":mr_sql", mr_sql)
            p[
                'value_source_value'] = "IF(person_id in (:mr_sql),'Multi-Racial',IF(value_source_concept_id not in (" \
                                        ":_ids), ':other_name',value_source_value)) as value_source_value".replace(
                ":_ids", _ids).replace(":other_name", other_name).replace(":mr_sql", mr_sql)
    ...
    def gender(self):
        """
            ...
            @NOTE : The table has fields with redundant information (shit design) i.e gender_source_value and
            gender_concept_id
            ...
            @param fields
        """
        sql = "SELECT concept_id,concept_name FROM :dataset.concept WHERE (vocabulary_id= 'Gender' AND concept_name " \
              "not in ('FEMALE','MALE') ) OR REGEXP_CONTAINS(concept_code,'_Man|_Woman')"
        sql = sql.replace(":dataset", self.dataset)
        r = self.client.query(sql)
        r = r.to_dataframe()
        ...
        _ids = ",".join(
            [str(value) for value in r[r['concept_name'] != 'OTHER']['concept_id'].tolist()])  # -- ids to generalize
        ...
        if self.table == 'person':
            ...
            #
            p[
                "gender_concept_id"] = "IF(gender_concept_id in ( :_ids ),:other_id,gender_concept_id) as " \
                                       "gender_concept_id".replace(
                ":_ids", _ids).replace(":other_id", other_id)
            p[
                "gender_source_value"] = "IF(gender_concept_id in (:_ids),:other_name,gender_source_value) as " \
                                         "gender_source_value".replace(
                ":_ids", _ids).replace(":other_name", other_name)
            # for name in p :
            #     index = fields.index(name)
            ...
            #
            p[
                'value_as_string'] = "IF(value_source_concept_id not in (:_ids),':other_name',value_as_string) as " \
                                     "value_as_string".replace(
                ":_ids", _ids).replace(":other_name", other_name)
            p[
                'observation_source_concept_id'] = "IF(value_source_concept_id not in (:_ids),:other_id," \
                                                   "observation_source_concept_id) as " \
                                                   "observation_source_concept_id".replace(
                ":_ids", _ids).replace(":other_id", other_id)
            p[
                'observation_source_value'] = "IF(value_source_concept_id not in (:_ids), ':other_name'," \
                                              "observation_source_value) as observation_source_value".replace(
                ":_ids", _ids).replace(":other_name", other_name)
            p[
                'value_source_value'] = "IF(value_source_concept_id not in (:_ids), ':other_name',value_source_value) " \
                                        "as value_source_value".replace(
                ":_ids", _ids).replace(":other_name", other_name)
        return self.get_fields(p)
    ...
    def __get_formatted_observations(self, _ids, other_name, other_id):
        ...
        p[
            ...
            ":_ids", _ids).replace(":other_name", other_name)
        p[
            'value_source_value'] = "IF(value_source_concept_id not in (:_ids), ':other_name',value_source_value) as " \
                                    "value_source_value".replace(
            ":_ids", _ids).replace(":other_name", other_name)
        return self.get_fields(p)
    ...
    def orientation(self):
        """
            This function will generalize sexual orientation on the observation table, this only applies to the
            observation table (for now)
            @filter    TheBasics_SexualOrientation
        """
        sql = "SELECT concept_id,concept_code,concept_name from :dataset.concept where REGEXP_CONTAINS(concept_code, " \
              "'Orientation_Straight|Orientation_None')"
        sql = sql.replace(":dataset", self.dataset)
        r = self.client.query(sql)
        r = r.to_dataframe()

        other_id = str(
            r[r['concept_code'] == Policy.TERMS.SEXUAL_ORIENTATION_NOT_STRAIGHT]['concept_id'].tolist()[0])  # --

        other_name = r[r['concept_code'] == Policy.TERMS.SEXUAL_ORIENTATION_NOT_STRAIGHT]['concept_code'].tolist()[0]
        #                     #--
        _ids = [str(value) for value in r[r['concept_code'] == Policy.TERMS.SEXUAL_ORIENTATION_STRAIGHT][
            'concept_id'].tolist()]  # -- ids to generalize
    ...
    def education(self):
        """
            Educattion should be in 5 categories provided by the concept_codes below. Because we do NOT have an
            unknown education level we will hard code it and set it's concept id to zero (No matching concept)
            @TODO:
            The data curation team should add this in the concept table (put in a request with Mark or Chun Yee)
        """
        sql = "SELECT concept_id,concept_code,concept_name from :dataset.concept WHERE concept_code in (" \
              "'HighestGrade_AdvancedDegree','HighestGrade_CollegeOnetoThree','HighestGrade_TwelveOrGED'," \
    ...
    def sex_at_birth(self):
        """
            ...
            @filter value_source_concept_id in (SELECT concept_id from :dataset.concept WHERE concept_code =
            'BiologicalSexAtBirth_SexAtBirth')
        """
        sql = "SELECT concept_id,concept_code,concept_name from :dataset.concept WHERE  concept_code in (" \
              "'SexAtBirth_Female', 'SexAtBirth_Male')"
        other_id = '0'
        other_name = 'Unknown'
        sql = sql.replace(":dataset", self.dataset)
        r = self.client.query(sql)
    ...
    def employment(self):
        ...
        """
        sql = "SELECT concept_id,concept_code,concept_name from :dataset.concept WHERE concept_code in (" \
              "'EmploymentStatus_OutOfWorkOneOrMore','EmploymentStatus_EmployedForWages'," \
              "'EmploymentStatus_OutOfWorkLessThanOne')"
        other_id = '0'
        other_name = 'Unknown'
        sql = sql.replace(":dataset", self.dataset)
        r = self.client.query(sql)
...
def initialization(client, dataset):
    ...
    if has_table == True:
        ...
        Logging.log(subject="composer", object="make.seed", action="evaluate",
                    value=r_i.num_rows / r['count'].values[0])
    ...
    if has_table == False:
        ...
        # sql = """
        #         SELECT person_id, DATE_DIFF(CURRENT_DATE,CAST(value_as_string as DATE) , DAY)+ CAST (720*rand() AS
        #         INT64) as seed
        ...
        #         FROM :i_dataset.observation WHERE person_id not in (SELECT person_id FROM :i_dataset.observation
        #         WHERE observation_source_value = 'ExtraConsent_TodaysDate' GROUP BY person_id)
        #         GROUP BY person_id
...
def finalize(**args): #client,i_dataset, o_dataset,table,o_table,fields):
    ...
    if len(oschema) == ischema_size:
        try:
            ...
            Logging.log(subject='composer', object='finalize', action='update.schema', value={"table": table.table_id,
                                                                                              "schema": [" ".join(
                                                                                                  [field.name,
                                                                                                   field.field_type])
                                                                                                         for field in
...
if __name__ == '__main__':
    ...
    COMPUTE_FIELDS = SYS_ARGS['config']['compute'][table] if 'compute' in SYS_ARGS['config'] and table in \
                                                             SYS_ARGS['config']['compute'] else {}
    ...
    #
    args = {"client": client, "vocabulary_id": 'PPI', "concept_class_id": ['Question', 'PPI Modifier'],
            "dataset": i_dataset, table: table, "remove": remove}
    container = [Shift(**args), DropFields(**args)]
    #
    # Let's see what we can do with the designated table, given our container of operations
    # Each item in the container is fully autonomous and will return a query that will have to be built by the
    # calling code
    # The reason for this is because the operations are already convoluted as is: separation of concerns (
    ...
    #   - dropping fields performs a projection of a table given fields suppressed (should probably be renamed).
    #   Date/TimeStamp fields will be automatically dropped if not specified
    #   - Dates are shifted and will/should be joined against the fields of the previous step
    #   - In the advent of observation table (meta and relational) an additional union is added to the construction
    #   process
    #
    ...
    if 'exclude-age' in CONSTANTS and 'person_id' in _fields:
        EXCLUDE_AGE_SQL = "person_id not in (SELECT person_id FROM :i_dataset.observation where " \
                          "observation_source_value = 'PIIBirthInformation_BirthDate' and DATE_DIFF(CURRENT_DATE, " \
                          "CAST(value_as_string AS DATE),YEAR) > :age)"
        EXCLUDE_AGE_SQL = EXCLUDE_AGE_SQL.replace(":age", str(CONSTANTS['exclude-age'])).replace(":i_dataset",
                                                                                                 i_dataset)
