...
while true; do
  case "$1" in
    --source_app_id) SOURCE_APPLICATION_ID=$2; shift 2;;
    --target_app_id) TARGET_APPLICATION_ID=$2; shift 2;;
...
do
  ...
  CP_CMD="bq cp --sync=${SYNC} -f ${SOURCE_DATASET}.${t} ${TARGET_DATASET}.${TARGET_PREFIX}${TARGET_TABLE}"
  echo $(${CP_CMD})
done
